...
public class PurifierContainer extends Container implements IMachineStateContainer {

	private final TileEntityPurifier tileEntity;

	public PurifierContainer(IInventory playerInventory, TileEntityPurifier tileEntity)
	{
	...
	private void addOwnSlots()
	{
		...
		addSlotToContainer(new SlotItemHandler(itemHandler, 1, 141, 35));
	}
	...
	public void detectAndSendChanges()
	{
		...
		{
			if(tileEntity.getProgress() != tileEntity.getClientProgress() || tileEntity.getEnergy() != tileEntity.getClientEnergy())
			{
				...
				for(IContainerListener listener : listeners)
				{
					if(listener instanceof EntityPlayerMP)
					{
						...
						int percent = 100 - tileEntity.getProgress() * 100 / PurifierConfig.MAX_PROGRESS;
						Messages.INSTANCE.sendTo(new PacketSyncMachineState(this.tileEntity.getEnergy(), percent), player);
					}
				}
			}
		}
