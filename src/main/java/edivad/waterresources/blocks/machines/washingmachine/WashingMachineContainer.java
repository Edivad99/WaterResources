...
public class WashingMachineContainer extends Container implements IMachineStateContainer {

	private final TileEntityWashingMachine tileEntity;

	public WashingMachineContainer(IInventory playerInventory, TileEntityWashingMachine tileEntity)
	{
	...
	private void addOwnSlots()
	{
		...
		addSlotToContainer(new SlotItemHandler(itemHandler, 0, 141, 35));
	}
	...
	public void detectAndSendChanges()
	{
		...
		{
			if(tileEntity.getProgress() != tileEntity.getClientProgress() || tileEntity.getEnergy() != tileEntity.getClientEnergy() || tileEntity.getFluidWaterAmount() != tileEntity.getClientFluidWaterAmount() || tileEntity.getFluidAUAmount() != tileEntity.getClientFluidAUAmount())
			{
				tileEntity.setClientEnergy(tileEntity.getEnergy());
				tileEntity.setClientProgress(tileEntity.getProgress());
				tileEntity.setClientFluidWaterAmount(tileEntity.getFluidWaterAmount());
				tileEntity.setClientFluidAUAmount(tileEntity.getFluidAUAmount());
				...
				for(IContainerListener listener : listeners)
				{
					if(listener instanceof EntityPlayerMP)
					{
						...
						int percent = 100 - tileEntity.getProgress() * 100 / WashingMachineConfig.MAX_PROGRESS;
						Messages.INSTANCE.sendTo(new PacketSyncMachineState(this.tileEntity.getEnergy(), percent, tileEntity.getFluidWaterAmount(), tileEntity.getFluidAUAmount()), player);
					}
				}
			}
		}
	...
	public void sync(int energy, int progress, int... fluids)
	{
		...
		this.tileEntity.setClientFluidWaterAmount(fluids[0]);
		this.tileEntity.setClientFluidAUAmount(fluids[1]);
	}
}
