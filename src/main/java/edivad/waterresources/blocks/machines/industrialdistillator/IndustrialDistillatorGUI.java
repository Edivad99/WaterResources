...
public class IndustrialDistillatorGUI extends GuiContainer {

	private static final ResourceLocation TEXTURES = new ResourceLocation(Main.MODID + ":textures/gui/industrial_distillator.png");
	private final TileEntityIndustrialDistillator tileEntity;

	public IndustrialDistillatorGUI(TileEntityIndustrialDistillator tileEntity, IndustrialDistillatorContainer container)
	{
		super(container);
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks)
	{
		...
		if(mouseX > guiLeft + 140 && mouseX < guiLeft + 161 && mouseY > guiTop + 10 && mouseY < guiTop + 77)
			drawHoveringText(Collections.singletonList("Distilled water: " + this.tileEntity.getClientFluidOutAmount() + " mB"), mouseX, mouseY, fontRenderer);
	}

	@Override
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY)
	{
		if(this.tileEntity.coolingPlateIsWorking())
			this.fontRenderer.drawString("Connected", (this.xSize / 2 - this.fontRenderer.getStringWidth("Connected") / 2) + 15, 8, 4210752);
	}
	...
	protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY)
	{
		...
		this.drawTexturedModalRect(this.guiLeft + 85, this.guiTop + 31, 176, 0, x, 17);
		...
		fluidTexture = mc.getTextureMapBlocks().getTextureExtry(ModLiquids.distilled_water.getStill().toString());
	...
	{
		int maxLiquidAmount = IndustrialDistillator.TANK_CAPACITY;
